...
`,
        youNoLongerHaveJetton: (ticker: string) => `You no longer hold tokens of "${ticker}", notifications for them have been stopped.`,
        notification: {
            x2: (ticker: string, rate: number, wallet: string) => `"${ticker}" x${+rate.toFixed(2)} in the wallet "${wallet}".`,
            x05: (ticker: string, rate: number, wallet: string) => `"${ticker}" x${+rate.toFixed(2)} in the wallet "${wallet}".`,
        },
    },
    button: {
